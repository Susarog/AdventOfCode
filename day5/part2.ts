...
data
  ...
  .forEach((input) => {
    ...
    const temp = arrayOfStacks[from].slice(
      arrayOfStacks[from].length - amountToMove,
      arrayOfStacks[from].length
    );
    arrayOfStacks[from] = arrayOfStacks[from].slice(
      0,
      arrayOfStacks[from].length - amountToMove
    );
    arrayOfStacks[to].push(...temp);
  });
